...
### Опционально: распознавание при паузе

![extension panel](docs/popup.png)

Нажмите на значок расширения и включите **«Распознавать при паузе»** —
после этого текст появляется сразу при паузе, без нажатия на значок.

Плата за это — распознавание при каждой паузе (≈0,5–1 с), поэтому **по умолчанию выключено**.
